...
void Graphics::DrawSpriteNonChroma(int x, int y, const RectI & rec, const Surface & s)
{
	DrawSpriteNonChroma(x, y, rec, GetScreenRect(), s);
}

void Graphics::DrawSpriteNonChroma(int x, int y, RectI rec, const RectI& clip, const Surface & s)
{
	assert(rec.left >= 0);
	...
	for (int sx = rec.left; sx < rec.right; ++sx)
	{
		...
		{
			PutPixel(x + sx - rec.left, y + sy - rec.top, s.GetPixel(sx, sy));
		}
	}
}
...
{
	DrawSprite(x, y, s.GetRect(), GetScreenRect(),s);
}

void Graphics::DrawSprite(int x, int y, const RectI & rec, const Surface & s)
{
	DrawSprite(x, y, rec, GetScreenRect(), s);
}

void Graphics::DrawSprite(int x, int y, RectI rec, const RectI & clip, const Surface & s, Color chroma)
{
	assert(rec.left >= 0);
	assert(rec.right <= s.GetWidth());
	assert(rec.top >= 0);
	...
	for (int sx = rec.left; sx < rec.right; ++sx)
	{
		for (int sy = rec.top; sy < rec.bottom; ++sy)
		{
			...
			if (pixel != chroma)
			{
				PutPixel(x + sx - rec.left, y + sy - rec.top, pixel);
			}
		}
	}
}
